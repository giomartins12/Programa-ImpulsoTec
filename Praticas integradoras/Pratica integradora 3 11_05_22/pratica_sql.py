...
def cadastra_veiculo():
    ...
    sql = "INSERT INTO tbl_oficina (modelo, marca, ano, placa) VALUES (%s, %s, %s, %s)"
...
def lista_veiculos():
    lista_veiculo = "SELECT * FROM tbl_oficina"
...
def lista_pesquisa_veiculo():
    ...
    lista_pesquisa = f"SELECT * FROM tbl_oficina WHERE {pesquisa_veiculo}= '{nome_pesquisa}'"
...
def apaga_veiculos():
    apaga_veiculo = "TRUNCATE tbl_oficina"
...
def apaga_veiculos_campo():
    ...
    apaga_veiculo = f"DELETE FROM tbl_oficina WHERE {pesquisa_apagar}= '{apaga_dado}'"
    cursor.execute(apaga_veiculo)
    db_connection.commit()
    print("\nApagando dados...")
    time.sleep(2)
